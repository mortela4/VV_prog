...
class CommandLayout(QtWidgets.QGridLayout):
    ...
    def add_status_indicator(self):
        ...
        self.status_out.setStyleSheet("QLineEdit {color: black; background-color: rgb(255, 255, 0)}")     # White text on yellow background is pretty hard to read ...
...
class App(QtWidgets.QWidget):
    ...
    def update_status_indicator(self, status: str):
        if status == "success":
            ...
            self.opt_set.status_out.setStyleSheet("QLineEdit {background-color: green}")       
        else:
            self.opt_set.status_out.setText("FAIL!")
            self.opt_set.status_out.setStyleSheet("QLineEdit {background-color: red}")       
