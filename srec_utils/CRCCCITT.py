...
class CRCCCITT(object):
    ...
    def calculate_accumulated_endian(self, input_data=None, prev_value=None):
        """
        ...
        try:
            ...
            byte_quad = bytearray()
            for c in input_data:
                if num_bytes % 4 == 0 and num_bytes > 0:
                    # uint32_val = int(byte_quad.encode('hex'), 16)
                    uint32_val = int.from_bytes(byte_quad, byteorder='little', signed=False)
                    byte_quad = bytearray()
                    for i in range(4):
